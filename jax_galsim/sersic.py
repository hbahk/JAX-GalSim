...
def calculate_b(n, invn, gamma2n, flux_fraction):
    ...
    invnsq = invn * invn
    b1 = 2.0 * n - 1.0 / 3.0
    b2 = b1 + (8.0 / 405.0) * invn + (46.0 / 25515.0) * invnsq + (131.0 / 1148175.0) * invn * invnsq
...
def calculate_truncated_scale(n, invn, b, trunc):
    ...
    if trunc <= jnp.sqrt(2.0):
        raise ValueError("Sersic truncation must be larger than sqrt(2)*half_light_radius.")
...
class Sersic(GSObject):
    ...
    def __init__(self, n, half_light_radius=None, scale_radius=None,
        ...
        if self._n < Sersic._minimum_n:
            raise _galsim.GalSimRangeError("Requested Sersic index is too small",
                                   self._n, Sersic._minimum_n, Sersic._maximum_n)
        if self._n > Sersic._maximum_n:
            raise _galsim.GalSimRangeError("Requested Sersic index is too large",
                                   self._n, Sersic._minimum_n, Sersic._maximum_n)

        if self._trunc < 0:
            raise _galsim.GalSimRangeError("Sersic trunc must be > 0", self._trunc, 0.)
        ...
        if half_light_radius is not None:
            ...
            if self._trunc == 0. or flux_untruncated:
                ...
                if self._trunc <= jnp.sqrt(2.) * self._hlr:
                    raise _galsim.GalSimRangeError("Sersic trunc must be > sqrt(2) * half_light_radius",
                                           self._trunc, jnp.sqrt(2.) * self._hlr)
    ...
    def _calculate_missing_flux_radius(self, missing_flux_frac):
        ...
        if self._n == 0.5:
            ...
            z2 = z1 + twonm1 * jnp.log(z) + twonm1 / z + (twonm1 * (2.0 * self._n - 3.0)) / (2.0 * z * z)
